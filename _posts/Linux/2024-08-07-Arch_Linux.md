...
![](static/GinZbJBsDozAFQxDaDncrojKn4e.jpg)
...
![](static/B05abv6DCoSBO5xQBhzc7SOxnib.png)
...
![](static/LDOkbfHFZoY3dpx4lORc7J1Qndc.png)
![](static/CYC5burNco9IyGxsXs1cevFcndb.png)

虚拟机配置完成，启动虚拟机，启动成功（接下来每当我以为系统安装完成重启之后还会反复看到这个画面，今晚梦里都是这个了）

![](static/F1ACb6lbgo6CqjxC7wXcp1benSg.png)
...
![](static/DYdtbGckqohe49xGi1RcdgVEnef.png)
...
![](static/AhhxbUpUSodqMJxyEHfcwA87nLb.png)

#### 进行分盘操作

最开始硬盘分布是这样的

![](static/EAOdbAaDoo8gkzxnWbLc542Wnth.png)

创建 GPT 分区

![](static/SsHub0nS1o8FIlxOa2FcY86CnVe.png)

创建根分区（第一次只分配了 10G，然后删除分区重新分了 40G 上去）

![](static/ZqmBbmdUEon7VdxV66XcNTPhnEZ.png)

创建 home 分区

![](static/L1QCbTpk6oJYYlxVhHgcKQIonnb.png)

分区状态最终打印如下

![](static/ORKubhru2otRGnxWceRc4Bfmnrb.png)

#### 挂载分区

挂载分区时失败了

![](static/DpVubN9mlopymtxJz9zcelCrnxc.png)

原因是没有将分区格式化为 ext4 文件系统，之后再挂载就没有报错了

![](static/QuKkbClBMoY0cvxqsnOclPTInEh.png)

接着给仓库镜像换成南大源 `https://mirror.nju.edu.cn/archlinux/$repo/os/$arch` 然后更新软件包缓存

![](static/J7IdbvG0Uo7gcbxfF3Pcn7Danje.png)

#### 安装三件套（姑且称呼为三件套吧）

安装 base linux linux-firmware 软件包

![](static/LbV8bKKDRoFZ6dxD1fmcpLOSnbc.png)

安装成功，但是中间出现了很多 WARNING，没有 ERROR 就先不管了

![](static/EdElbABXCoYkCVxHCXLc7LBdnCg.png)

生成 fstab

![](static/LN1JbMkyto80nmxBZc7cD1zunLd.png)

#### 进入 arch-chroot

设置时区和硬件时间

![](static/UYMJbTRtQo3vNrxLTisckKO8neb.png)

在新系统中安装 vim 后，编辑/etc/locale.conf

![](static/XA2Ob2j4Qokgx5xZTeacicO3nlc.png)
...
![](static/TYk2bIKLqoHvUVxPAZzcshkMnob.png)
...
![](static/A7drb7aawo5DEnxBpQacJUjNnzh.png)

安装 EFI 启动管理器

![](static/Yd3rboc8Ho1oUgxA9B1ctUqhnTf.png)

配置/boot/loader/loader.conf 启动文件

![](static/ROhXbpB02oPznyxvXk1cYvGWnrJ.png)

配置/boot/loader/entries/arch/conf

![](static/Xe3ubUmPKoyJIAxyYKwcHkM2nwc.png)
